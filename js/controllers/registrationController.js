app.controller("registrationController",function($scope,$http,$location,$routeParams,$filter){
    ...
    $http.get(RCC_API_URI , {params: {"token" : $scope.params.token}}).then(function(response) {
        ...
        $scope.isEventDataAvailable = true;
                var _participant = $filter('filter')($scope.event.participants, { Id : $scope.participant.Id});
    ...
    $scope.registerParticipant = function(){
        ...
        $http.post(RCC_API_URI, postParams).then(function(response){
            ...
            var _participants = $filter('filter')($scope.event.participants, { Id : postParams.Id});
